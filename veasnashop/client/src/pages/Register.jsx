...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!agree) {
      setError("Please agree to the Terms of Service & Privacy Policy to create your account.");
      return;
    }
    setError("");
    setLoading(true);
    try {
      const data = await register(`${firstName} ${lastName}`.trim(), email, password);
            ...
            <div className="flex flex-col gap-8">
              <div className="flex items-center justify-between gap-4">
                <div className="inline-flex p-1 rounded-full bg-surface-container-low" role="tablist">
                  <Link to="/login" className="px-6 py-2 rounded-full font-label-lg text-label-lg transition-all text-on-surface-variant hover:text-primary" type="button">
                    Sign In
                  </Link>
              ...
              <div className="flex flex-col gap-6">
                <div className="flex flex-col gap-1.5">
                  ...
                  <p className="font-body-md text-body-md text-on-surface-variant">
                    Create your collector account for curated living, early releases, and apothecary notes.
                  </p>
                </div>
                ...
                <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                    <div className="flex flex-col gap-1.5">
                      <label className="font-label-md text-label-md text-on-surface font-medium" htmlFor="reg-first-name">First Name</label>
                      <input
                        className="w-full px-4 py-3 bg-surface-container-lowest rounded-xl font-body-sm text-body-sm text-on-surface placeholder:text-outline-variant outline-none shadow-sm focus:bg-surface-container-low transition-colors"
                        id="reg-first-name"
                        placeholder="Aria"
                        type="text"
                        value={firstName}
                    ...
                    <div className="flex flex-col gap-1.5">
                      <label className="font-label-md text-label-md text-on-surface font-medium" htmlFor="reg-last-name">Last Name</label>
                      <input
                        className="w-full px-4 py-3 bg-surface-container-lowest rounded-xl font-body-sm text-body-sm text-on-surface placeholder:text-outline-variant outline-none shadow-sm focus:bg-surface-container-low transition-colors"
                        id="reg-last-name"
                        placeholder="Thorne"
                  ...
                  <div className="flex flex-col gap-1.5">
                    <label className="font-label-md text-label-md text-on-surface font-medium" htmlFor="reg-email">Email Address</label>
                    <div className="relative flex items-center">
                      ...
                      <input
                        ...
                        id="reg-email"
                        placeholder="aria@sanctuary.com"
                        type="email"
                        value={email}
                  ...
                  <div className="flex flex-col gap-1.5">
                    <label className="font-label-md text-label-md text-on-surface font-medium" htmlFor="reg-password">Create Master Password</label>
                    <div className="relative flex items-center">
                    ...
                    <div className="flex flex-col gap-1 mt-1">
                      <div className="grid grid-cols-4 gap-1.5 h-1.5 w-full">
                        ...
                        <div className="rounded-full bg-secondary" />
                        <div className="rounded-full bg-secondary" />
                        <div className="rounded-full bg-surface-container-high" />
                      </div>
                      <div className="flex items-center justify-between text-[11px] font-label-sm text-on-surface-variant">
                        <span>Mindful strength: <strong className="text-secondary">Botanical-Grade</strong></span>
                        <span>Contains 1 symbol &amp; uppercase</span>
                      </div>
                    </div>
                  </div>
                  <div className="flex flex-col gap-2.5 pt-1">
                    <label className="flex items-start gap-2.5 cursor-pointer select-none">
                      <input
                        checked={dispatch}
                        className="w-4 h-4 mt-0.5 rounded bg-surface-container text-primary accent-primary cursor-pointer flex-shrink-0"
                        type="checkbox"
                        onChange={(e) => setDispatch(e.target.checked)}
                      />
                      <span className="font-body-sm text-body-sm text-on-surface-variant leading-tight">
                        Join our <strong>Seasonal Capsule Dispatch</strong> (gentle monthly philosophical essays, seasonal harvests, never spam).
                      </span>
                    </label>
                    <label className="flex items-start gap-2.5 cursor-pointer select-none">
                      <input
                      ...
                      />
                      <span className="font-body-sm text-body-sm text-on-surface-variant leading-tight">
                        I agree to the <span className="text-primary underline decoration-outline-variant font-medium">Terms of Domestic Sanctuary</span> &amp; <span className="text-primary underline decoration-outline-variant font-medium">Privacy Policy</span>.
                      </span>
                    </label>
                  </div>
                  <button
                    className="mt-2 w-full py-3.5 px-6 rounded-xl bg-primary hover:bg-primary-container text-on-primary font-label-lg text-label-lg font-semibold tracking-wide transition-all shadow-md flex items-center justify-center gap-2 group"
                    type="submit"
                    disabled={loading}
                  >
                    <span>{loading ? "Creating account..." : "Create Veasna Account"}</span>
                    {!loading && <Icon name="spa" className="text-[18px] group-hover:translate-x-0.5 transition-transform" />}
                  </button>
                </form>
              </div>
            </div>
